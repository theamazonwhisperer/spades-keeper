...
function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Race against 10 s so a cold/slow Supabase never freezes the UI on mobile.
      // A hung promise never rejects, so try/catch alone doesn't protect us here.
      const withTimeout = <T,>(p: Promise<T>, fallback: T): Promise<T> =>
        Promise.race([p, new Promise<T>(r => setTimeout(() => r(fallback), 10_000))]);

      // Run both fetches in parallel so a cold-start penalty isn't paid twice.
      const [cloud, sharedGames] = await Promise.all([
        withTimeout(loadCloudState(user.id), null).catch(() => null),
        withTimeout(loadSharedGames(user.id), []).catch(() => []),
      ]);
      ...
      if (sharedGames.length > 0) {
        ...
        // Fire-and-forget — don't let cleanup block setSyncing(false)
        clearSharedGames(user.id, sharedGames.map(g => g.id)).catch(() => {});
      }
