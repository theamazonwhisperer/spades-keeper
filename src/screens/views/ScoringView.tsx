...
export default function ScoringView() {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      setShareError(err instanceof Error ? err.message : 'Failed to share scorecard');
    }
  };
        ...
        <Box sx={{ mt: 2 }}>
          <TextField
            ...
            onChange={e => {
              ...
              const rounds = game?.rounds.filter(r => r.isComplete) ?? [];
              const roundNum = rounds[rounds.length - 1]?.roundNumber;
              if (roundNum != null) addRoundNote(roundNum, e.target.value);
            }}
