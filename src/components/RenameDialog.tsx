...
export default function RenameDialog({ open, onClose }: Props) {
  ...
  const handleSave = () => {
    ...
    // Update team names (only if changed) — skip for 3-player since team = player
    if (!is3Player) {
      currentGame.teams.forEach((team, idx) => {
        if (teamNames[idx] !== team.name) renameTeam(team.id, teamNames[idx]);
      });
    }
    // Update player names
    currentGame.players.forEach((player, idx) => {
      if (playerNames[idx] !== player.name) {
        ...
        // In 3-player mode keep the team name in sync with the player name
        if (is3Player) renameTeam(currentGame.teams[player.teamIndex].id, playerNames[idx]);
      }
    });
    onClose();
  };
  ...
  const canSave = (is3Player || teamNames.every(n => n.trim())) &&
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      ...
      <DialogContent dividers>
        {is3Player ? (
          // 3-player: just list player names
          currentGame.players.map((player, idx) => (
            <Box key={player.id} sx={{ mb: idx < currentGame.players.length - 1 ? 2 : 0 }}>
              <Typography
                variant="caption"
                sx={{
          ...
          // 4-player: team name + players per team
