...
export default function RenameDialog({ open, onClose }: Props) {
  ...
  const handleSave = () => {
    // Guard against race condition: if currentGame is null, don't save
    if (!currentGame) return;

    // Update team names (only if changed)
    if (teamNames[0] !== currentGame.teams[0].name) renameTeam(currentGame.teams[0].id, teamNames[0]);
    if (teamNames[1] !== currentGame.teams[1].name) renameTeam(currentGame.teams[1].id, teamNames[1]);
