...
export default function ScoreHistoryTable({ game, onEditRound }: Props) {
  ...
  return (
    <Box sx={{ overflowX: 'auto' }}>
      <Table size="small" sx={{ minWidth: teams.length > 2 ? 360 : 280 }}>
        <TableHead>
          <TableRow>
        ...
        <TableBody>
          {completedRounds.map(round => (
            ...
            >
              ...
              {teams.map(team => {
                ...
                const sPositive = (ts?.roundTotal ?? 0) >= 0;
                return [
                  ...
                  <TableCell
                    ...
                    sx={{
                      ...cellSx,
                      fontWeight: 700,
                      color: sPositive ? theme.palette.success.main : theme.palette.error.main,
                    }}
                    align="center"
                  >
          ...
          {lastRound.teamScores.some(ts => ts.cumulativeBags >= 10) && (
            <TableRow>
              ...
              {teams.map(team => {
                ...
                return [
                  <TableCell key={`${team.id}-fine-empty1`} colSpan={2} />,
                  <TableCell key={`${team.id}-fine-bags`} sx={{ ...cellSx, color: theme.palette.error.main, fontWeight: 700 }} align="center">
                    {ts?.cumulativeBags ?? 0}
                  </TableCell>,
                ];
              })}
          ...
          <TableRow sx={{ bgcolor: alpha(theme.palette.primary.main, 0.08) }}>
            ...
            {teams.map(team => (
              [
                <TableCell key={`${team.id}-total-empty`} colSpan={2} />,
                <TableCell key={`${team.id}-total-empty2`} colSpan={1} />,
              ]
            ))}
          </TableRow>
