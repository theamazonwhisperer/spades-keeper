...
export default function GameSettingsDialog({ open, onClose }: Props) {
  ...
  useEffect(() => {
    // Only sync from currentGame.settings when the dialog transitions from closed to open.
    // Ignoring currentGame changes while the dialog is already open prevents background
    // cloud sync updates from silently discarding in-progress user edits.
    if (open && !prevOpenRef.current && currentGame) {
