...
export function checkGameOver(
  ...
  if (settings.maxRounds && completedRounds.length >= settings.maxRounds) {
    ...
    // Tied scores — declare a draw with no winner
    if (sorted.length >= 2 && sorted[0].cumulativeScore === sorted[1].cumulativeScore) {
      return { isOver: true, winnerId: undefined };
    }
  ...
  if (overTarget.length > 0) {
    ...
    // Both teams crossed the target in the same round with equal scores — draw
    if (overTarget.length >= 2 && overTarget[0].cumulativeScore === overTarget[1].cumulativeScore) {
      return { isOver: true, winnerId: undefined };
    }
